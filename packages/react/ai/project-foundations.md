...
| `wireframe-redacted` | `.a1-theme-wireframe-redacted` | The Wireframe theme with **copy redacted** — running text is hidden and replaced by solid grey **bars, one per line** (darker for headings, muted for body), so a reviewer reads layout, hierarchy, and density rather than the words. Same greyscale/no-shadow/zero-radius/monospace token foundation as Wireframe (its `overrides` are a copy — the theme-config guard forbids referencing another theme's files). The redaction itself is **not** a token override (theme `styles/*.json` can only style the `<html>` selector, not descendants): it lives in the hand-authored `apps/a1-web/src/priorityGuide/wireframe-redacted.css`, imported by a1-web, which makes text `transparent` and paints a `repeating-linear-gradient` bar per line. Icons, form controls, code, and inline-editable text are excluded so the wireframe stays operable. Reached from the Priority Guide editor's **Redacted** preview button. Light-mode only; web only |
