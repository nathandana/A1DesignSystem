...
9. **a1-web configurator + editor registry** — Add (or update) the component's page and interactive **configurator** under `apps/a1-web/src/pages/components/` so **every prop is adjustable in the live editor** and the generated code snippet stays correct, and register the page in the Components menu (`apps/a1-web/src/pages/Components.jsx`) per the menu hierarchy in `components.md`. Also update the Projects/Patterns editor surfaces in `apps/a1-web/src/editor/`: `componentRegistry.ts` for rendering, `componentCatalog.ts` for Add-panel availability, `EditorPropsPanel.jsx` configurator bridges for selected-node editing, and `pageTypes.ts` / `a1-agent-brief.md` when the JSON contract changes. A component or prop that isn't in the configurator and editor bridge is invisible to people building projects and patterns.
...
6. **a1-web configurator + project/pattern editor bridge** — Expose the new prop in the component's a1-web configurator: add a control for it in the **Configure** panel (and a row in the **Properties** table) under `apps/a1-web/src/pages/components/` so it's adjustable in the live editor and reflected in the emitted code snippet. **Give the control helper text** that explains the prop, wired through the shared **Helper-text switch** — pass it via the `helper` prop on the shared kit controls (driven by `ConfigHelpContext` / `WithHelp`) so it appears only when the user turns Helper text on. Never use a raw always-on `hint` for prop guidance; if a control doesn't yet support `helper`, add that support rather than bypassing the switch. Then update the Projects/Patterns editor bridge (`apps/a1-web/src/editor/EditorPropsPanel.jsx`) so selected-node editing can read/write the prop, and update `componentCatalog.ts` defaults when the Add panel needs the new behavior. A prop that isn't wired into the configurator and editor bridge is invisible to the people building with the editor; this step is **not optional**.
...
9c. **End every ticket with a CSS review.** Before declaring a ticket done, answer this explicitly: "For anything you have done in this session, justify any custom styling or CSS you have written. What should be included in the design system, what should have had better guidance, and what, if anything, is legitimate CSS that needed to be written." If no CSS was written, say so.
