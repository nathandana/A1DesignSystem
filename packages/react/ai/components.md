...
> **Figma Radio Group component:** A1-1403 created the Radio Group component set on the Radio Group page (`node 283:1121`) with a `Size` axis (`comfortable`, `default`, `compact`). Its `Radio Items` slot composes the separate Radio Option component set, which independently exposes the same `Size` axis alongside `selected` and visual `state` values. Editable parent properties cover the legend, helper, and required marker. Both sets use scoped `radioGroup/*` component variables and shared Light/Dark Color variables; the dark-mode validation frame inherits the Color collection mode rather than using a Dark variant. Code Connect template: `packages/figma/code-connect/RadioGroup.figma.ts`. Inline layout, dynamic options, input naming, controlled value, callbacks, error/disabled parent states, and native radio semantics remain documented Figma gaps in `packages/react/ai/figma-workflow.md`.
