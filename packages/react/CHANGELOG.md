...
- **Grid item span export parity** — added the existing `gridItemSpanClassNames` runtime export to the package root type declarations and shipped guideline export list, keeping the public JavaScript, TypeScript, and documentation surfaces aligned.
...
- **Shipped agent guidelines** — the npm tarball now includes the five companion guideline files that `guidelines/Guidelines.md` has always referenced but which never existed: `setup.md` (install, required CSS import order, fonts, PostCSS custom-media config, providers), `components.md` (every named export with key props and composition rules), `icon-discovery.md` (icon naming, verification, sizes, `custom:*` icon fonts), `tokens.md` (token tiers, theming, dark-mode/inverse contract), and `styles.md` (layout-first law, spacing scales, breakpoints, utilities). The hub's stale inline export table was replaced with a names-only category list of all `src/index.js` exports, and its theming table now covers every shipped theme.
- **`npm run pack:check`** (repo root) — `scripts/verify-pack.mjs` dry-packs this package and fails if a shipped module imports a file missing from the tarball, a `package.json` entry point doesn't ship, a guidelines file is missing, or the Guidelines export list drifts from `src/index.js`. Run it before every publish.
- **`./utilities/sr-only.css` export** — the visually-hidden utility stylesheet shipped in the tarball but was not resolvable through the exports map; it now has a subpath export like the spacing and width utilities.
- **Package metadata + shipped changelog** — added `repository` (with `directory`), `bugs`, and `homepage` to package.json, and `CHANGELOG.md` now ships in the tarball so consumers can see what changed between versions.
- **Type definitions completed** — `index.d.ts` had drifted 20 exports behind `index.js` (`BottomSheet`, `Canvas`/`Node`/`NodeConnector`/`CanvasEdge`, `Stat`, `TopHeader`, `TokenSelect`, `LabelsProvider`/`useLabel`, `DataTableFilters`, `SearchField`, `ZIP_MASKS`, and the Date/Time/Phone/Zip/CreditCard fields were all untyped at the package root). Rewrote it to mirror `index.js` 1:1 and added the nine missing `.d.ts` files (`DateField`, `TimeField`, `PhoneField`, `ZipField`, `CreditCardField`, `Labels`, `TokenSelect`, `TopHeader`, `DataTableFilters`). `npm run pack:check` now fails on index-level declaration drift and on `index.d.ts` references to modules with no shipped `.d.ts`.
- **`onChange` conventions documented** — `guidelines/components.md` now states the signature convention explicitly: field-family components pass the DOM event, selection controls pass the value, and `Switch` passes `(checked, event)`. (Unifying the signatures is a breaking change deferred to the next major — tracked in `TODO.md`.)
...
- **README corrected** — the published README used a wrong install name (`@a1-design-system/react` instead of `@gtivr4/a1-design-system-react`) and its usage example omitted the required `tokens.css` import — without which every component renders unstyled, since `themes.css` contains no `:root` token definitions. Rewritten with the real package name, the three-import CSS order, font loading (Material Symbols + Inter), the PostCSS custom-media requirement, and pointers to the shipped `guidelines/` docs.
