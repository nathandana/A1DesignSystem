...
export type TextListSize = 'xs' | 'sm' | 'md' | 'lg' | 'xl';
export type TextListColor = 'default' | 'muted';
export type TextListVariant = 'unordered' | 'ordered' | 'icon' | 'divider';
export type TextListAs = 'ul' | 'ol';
export type TextListSpacing = 'sm' | 'md' | 'lg';
export type TextListIcon = IconSource | null;

export interface TextListProps {
  as?: TextListAs;
  size?: TextListSize | Partial<Record<Breakpoint, TextListSize>>;
  color?: TextListColor;
  icon?: TextListIcon;
  variant?: TextListVariant;
  marginBottom?: TextListSpacing;
...
interface TextListContextValue {
  as: TextListAs;
  color: string;
  dividerColor: string;
...
const SIZES: TextListSize[] = ['xs', 'sm', 'md', 'lg', 'xl'];
const GAPS: Record<TextListSize, number> = {
...
const MARGIN_BOTTOM: Record<TextListSpacing, number> = {
...
function isResponsiveSize(size: TextListProps['size']): size is Partial<Record<Breakpoint, TextListSize>> {
  return Boolean(size && typeof size === 'object' && !Array.isArray(size));
}

function resolveBaseSize(size: TextListProps['size']): TextListSize {
  if (!isResponsiveSize(size)) return size && SIZES.includes(size) ? size : 'md';
  return size.xs && SIZES.includes(size.xs) ? size.xs : 'md';
}

function renderIcon(icon: Exclude<TextListIcon, null>, color: string, fontSize: number, lineHeight: number) {
...
export function TextList({
  ...
  const context: TextListContextValue = {
    as,
    ...
    <TextListContext.Provider value={context}>
      ...
      >
        {React.Children.toArray(children).map((child, index) => (
          <TextListContext.Provider key={(child as React.ReactElement).key ?? index} value={{ ...context, as }}>
            {React.isValidElement(child) ? React.cloneElement(child as React.ReactElement<any>, { index }) : child}
...
export function TextListItem({ icon: itemIcon, children, style, textStyle, index = 0 }: TextListItemProps & { index?: number }) {
