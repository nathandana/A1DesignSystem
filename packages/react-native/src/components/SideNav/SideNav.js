...
import { Animated, Dimensions, LayoutAnimation, Modal, Platform, Pressable, ScrollView, StyleSheet, Text, TouchableWithoutFeedback, View, } from 'react-native';
...
export function SideNavItem({ label, icon, active = false, badge, onPress }) {
    ...
    return (<Pressable accessibilityRole="menuitem" accessibilityState={{ selected: active }} onPress={onPress} style={({ pressed }) => [
            styles.item,
...
export function SideNavGroup({ label, icon, defaultOpen = false, open: controlledOpen, onOpenChange, children, }) {
    const depth = useContext(DepthCtx);
    ...
    return (<View>
      <Pressable accessibilityRole="button" accessibilityState={{ expanded: isOpen }} onPress={toggle} style={({ pressed }) => [
            styles.item,
            { paddingLeft: t.paddingInline + t.indent * depth, borderRadius: theme.navItemRadius },
        ...
        <Animated.Text style={[styles.chevron, { color: colors.textMuted, fontSize: closeIconFontSize, lineHeight: closeIconFontSize + 2, transform: [{ rotate: chevronRotate }] }]}>
          ›
        </Animated.Text>
      </Pressable>

      {isOpen && (<DepthCtx.Provider value={depth + 1}>
...
function Panel({ slideAnim, scrimAnim, onClose, header, footer, children, style }) {
    ...
    return (<>
      ...
      <Animated.View style={[
        ...
        <View style={[styles.panelHeader, { borderBottomColor: colors.borderSubtle, paddingTop: insets.top + spacingTokens.component.sideNav.paddingBlock }]}>
          ...
          <Pressable onPress={onClose} accessibilityRole="button" accessibilityLabel="Close navigation" style={({ pressed }) => [styles.closeBtn, { borderRadius: theme.navItemRadius }, pressed && { backgroundColor: colors.pressedBg }]}>
        ...
        {footer && (<View style={[styles.panelFooter, { borderTopColor: colors.borderSubtle, paddingBottom: Math.max(t.paddingBlock, insets.bottom) }]}>
            {footer}
          </View>)}
      </Animated.View>
    </>);
}
export function SideNav({ open, onClose, children, header, footer, style }) {
    ...
    if (!visible)
        return null;
    return (<Modal visible={visible} transparent animationType="none" onRequestClose={onClose} statusBarTranslucent={Platform.OS === 'android'}>
      <Panel slideAnim={slideAnim} scrimAnim={scrimAnim} onClose={onClose} header={header} footer={footer} style={style}>
        {children}
      </Panel>
    </Modal>);
}
