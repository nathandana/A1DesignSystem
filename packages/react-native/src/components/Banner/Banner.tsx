...
import { renderIconRenderer, type IconRenderer } from '../../utils/materialIcon';
import { IconButton } from '../IconButton/IconButton';

export type BannerStatus = 'neutral' | 'info' | 'success' | 'warn' | 'error';
export type BannerVariant = 'inline' | 'system';
type LegacyBannerVariant = Exclude<BannerStatus, 'neutral'>;

export interface BannerProps {
  title?: string;
  children: React.ReactNode;
  variant?: BannerVariant | LegacyBannerVariant;
  status?: BannerStatus;
  icon?: IconRenderer;
...
export function Banner({
  ...
  const resolvedVariant: BannerVariant = isLegacyStatus ? 'inline' : (variant as BannerVariant);
  const resolvedStatus: BannerStatus = status ?? (isLegacyStatus ? (variant as LegacyBannerVariant) : 'neutral');
  const isSystem = resolvedVariant === 'system';
  const statusColors = colors.badge[resolvedStatus];
