...
function getItems(page, total, siblings) {
...
export function Pagination({ page, totalPages, onChange, siblings = 1, size = 'md' }) {
    ...
    function NavBtn({ dir }) {
        ...
        return (<Pressable onPress={() => onChange(dir === 'prev' ? page - 1 : page + 1)} disabled={disabled} accessibilityRole="button" accessibilityLabel={dir === 'prev' ? 'Previous page' : 'Next page'} style={({ pressed }) => [
                styles.item,
                { width: dim, height: dim, borderRadius: radius, borderColor: 'transparent' },
                pressed && !disabled && { backgroundColor: colors.pressedBg },
                disabled && { opacity: 0.35 },
            ]}>
        <MaterialIcons name={dir === 'prev' ? 'chevron-left' : 'chevron-right'} size={iconSize} color={colors.textDefault}/>
      </Pressable>);
    }
    return (<View style={styles.row}>
      <NavBtn dir="prev"/>

      {items.map((item, idx) => item === 'ellipsis' ? (<Text key={`e${idx}`} style={[styles.ellipsis, { height: dim, color: colors.textMuted, fontSize: fs }]}>
            …
          </Text>) : (<Pressable key={item} onPress={() => item !== page && onChange(item)} accessibilityRole="button" accessibilityLabel={`Page ${item}`} style={({ pressed }) => [
                styles.item,
                {
