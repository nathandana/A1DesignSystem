...
- **Backlog filters — moved into the app's right-hand panel, persist + clear all** (A1-154) — **sort and the Type / Priority / Size / Scope filters moved into the app's right-hand aside rail** — the same `PageLayout` aside slot the editor/theme/component configurators use (portaled into `#a1-web-backlog-aside-slot`), so it spans the whole page and **auto-moves into a “Filters” BottomSheet at xs/sm**. The board area keeps only the swimlane view toggles + the board. The filters **persist across reloads** (saved to `localStorage` as `a1-backlog-filters`, with the sort and search), so a working view sticks, and a **“Clear filters (n)”** action in the panel resets every filter + the search in one click; the panel heading shows the active-filter count. No new filter dimensions and no overlap with the My-queue tab. `src/pages/Backlog.jsx` (`DEFAULT_FILTERS` / `loadFilterState` / `countActiveFilters` + a persist effect + a `createPortal` into the aside slot) and `main.jsx` (the backlog aside slot + BottomSheet title); built from existing A1 components — no design-system component changed.
