...
- **Backlog filters — persist + clear all** (A1-154) — completes the board's filters (Type / Priority / Size / Scope). They now **persist across reloads** (saved to `localStorage` as `a1-backlog-filters`, alongside the sort and the search), so a working view sticks. A **“Clear filters (n)”** button appears in the filter row whenever any filter or the search is narrowing the board, showing how many are active and resetting them all (filters + search) in one click. No new filter dimensions and no overlap with the My-queue tab — just the missing management affordances. `src/pages/Backlog.jsx` only (`DEFAULT_FILTERS` / `loadFilterState` / `countActiveFilters` + a persist effect + a `tertiary` Button); no design-system component changed.
