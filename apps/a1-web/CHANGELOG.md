...
- **Figma Choice Option audit support** — the A1:Figma audit now treats Choice Option instances as valid private Choice Group implementation components, matching Radio Option and Checkbox Option. Correct Choice Group tiles contribute A1 coverage without being exposed as standalone JSON nodes and no longer create major element-support findings.

- **Figma Figure image sync across page flows** — local Image Library `a1img://…` references now carry their PNG/JPEG/GIF bytes when an editor page is sent to Figma or pulled from the plugin Page Editor, matching the existing JSON Playground handoff. The localhost bridge keeps the sidecar volatile, omits it from the workspace manifest and returns it only for the selected page; page JSON continues to store only the stable image ID. The Figma controller uses a sandbox-safe byte codec when applying the nested Figure image fill. Cloud-backed IDs additionally resolve from the existing public A1 Supabase Storage origin when only JSON reaches the plugin, while IndexedDB-only images continue to use the sidecar. Handoffs support up to eight local Figure images and 4 MB total per page.

- **Visit analytics visualizations and request context** — added visit, unique-IP, page-view and average-duration metrics; 14-day, device and top-page charts; and a world map with a location-count table. New sessions store a whitelisted set of Netlify geolocation/request/site/deploy fields, Netlify's agent category and browser-reported User-Agent/language/Client Hints so device, browser and platform can be reviewed in the session dialog. Added a deterministic browser-only sample mode using documentation IP ranges, plus a follow-up Supabase migration and server/summary tests. The map uses local `world-atlas` geography through `react-simple-maps`; no third-party map request is made at runtime.

- **Visit analytics page and session details** — moved first-party visit analytics from Administration to the administrator-only `/admin/analytics` page. Each visit now opens a session-details dialog with account association, complete timestamps, duration, every IP observed, the ordered page timeline and an on-demand server-side ipapi.co lookup for approximate location, time zone and network ownership. IP enrichment is not stored, lookup failures preserve the underlying session, and the user-management page no longer loads visit records.

- **Section and Card custom surface colors** (A1-2537) — the Section and Card configurators' surface pickers gained the five new named surfaces (`accent`, `info`, `success`, `warn`, `error`) plus a "Custom" option that opens a small dialog (`CustomColorDialog`) with a hex `TextField` and native color-swatch picker for an arbitrary CSS color. Both pickers turned on `ToolbarGroup`'s `overflow` so the original surface set stays visible in the toolbar row while the new options move into an overflow menu on narrow containers.

- **Contrast check tool** (A1-2538) — added a manually-triggered "Contrast check" toolbar action to the page editor's edit view. It scans the live rendered canvas (the whole page, or just the selected element and its contents) for WCAG AA text-contrast issues, reading real computed colors so results reflect the current theme, color mode, and any overrides exactly as painted. Reports a summary and a findings table with the computed ratio, applicable AA minimum, pass/review status, and specific fix guidance per element. Non-text contrast (borders, focus rings, icon-only controls) and checking every theme/breakpoint automatically are out of scope for this pass and called out in the dialog.

- **Basic visit analytics** — added a first-party visit table to Administration with the IP addresses seen during each visit, signed-in account email when available, ordered route paths, page-view count, start time and approximate duration. A same-origin Netlify function reads the client IP from trusted request context and writes to a browser-inaccessible Supabase table; route query strings and page contents are not collected. Visits use a browser-tab session with a 30-minute inactivity boundary and a 30-second heartbeat. Added localized copy, an idempotent migration and server-boundary tests.

- **User-level access** (A1-405) — replaced the hosted app's all-or-nothing sign-in gate with cumulative Guest, User, Editor and Administrator roles sourced from trusted Supabase account metadata. Public visitors can read system guidance, use simplified release notes and try the browser-local page builder; signed-in users gain shared workspace tools and detailed release notes; Backlog and governance editors require Editor; Theme editor, Administration and Virtual team require Administrator. Restricted navigation, global search, shortcuts, providers and direct URLs now follow one tested policy. The Administration page securely lists accounts, opens detailed profiles, sends invitations, edits roles, deletes other accounts with exact-value confirmation, shows complete per-account lifecycle history, shows the 50 most recent access changes and lists all successful A1 password sign-ins recorded after login tracking is enabled. The administrator-authenticated Netlify function keeps its service key server-only, blocks self-demotion and self-deletion, and limits Auth profile data returned to the browser. Added localized access and user-management copy, browser-inaccessible account and login audit tables, editor-only Backlog row-level security, editor-only workspace-label writes, migrations for existing Supabase workspaces, server-policy tests and an access matrix with sequenced follow-ups.
...
- **Toolbar configurator — overflow pattern** (A1-354) — exposed the design-system Toolbar `overflow` prop so the configurator can preview/snippet the standard whole-toolbar More menu pattern for narrow containers.

- **Foundations — Motion examples** — added an Examples tab to the Motion foundation page with tokenized duration and easing demos, including replayable tracks that use semantic motion duration/easing tokens and respect reduced-motion settings.

- **Get started — Web components setup** — added Web components to the Package setup tabs with current `@gtivr4/a1-design-system-web` installation guidance, Lit peer dependency notes, custom-element imports for Button and Snackbar, token/theme CSS requirements, AI context files, and a framework-agnostic code example.

- **Image library — standalone feature** — moved the Image library view from `projects/ImageLibraryView.jsx` to `image-library/ImageLibraryView.jsx`, removed Projects from its breadcrumb, and switched the header to the shared `PageTitleArea` pattern wrapper so the page is presented as a standalone reusable asset library rather than a Projects subpage.

- **Patterns — page title area** (A1-355) — formalized the a1-web Page title area as a built-in governed pattern for page tops, including breadcrumb, title, supporting text, and action controls on the standard panel/accent header band. Added a shared app `PageTitleArea` wrapper and migrated the Patterns, Blog, and Projects page headers to it. The existing Page header pattern now composes the Page title area through `PatternRef`.

- **Color ramps** (A1-371) — fixed the highlighted color-ramp token spelling, including component token aliases and a1-web Section gradient swatches.

- **Search keyboard shortcut** — `'` now activates the visible on-page search field. Backlog search, Help search, Blog search, Components search, Label editor search, and DataTable searches can all be focused from the keyboard; `/` and `Cmd/Ctrl+K` continue to open global search directly.
...
- **Shared focus ring color** — standardized component focus-ring color usage on the shared `--component-button-focus-ring` token and raised that token to the stronger focus-ring value generated as `#5916b5` in the default bundle. Components that previously used action/accent/currentColor focus outlines now consume the shared focus token, while preserving their local width and offset. The Button accessibility report focus row now reflects the real computed `9.09:1` contrast against the default page surface.

- **Button accessibility report showcase** — replaced the Button component's Accessibility tab placeholder with a full reference report format: summary status cards, contrast rows for variants/states/focus, generated target-size rows for WCAG 2.5.5 / 2.5.8, keyboard controls, screen reader expectations, WCAG mapping, test matrix, manual checklist, implementation notes, and release-gate guidance.

- **Component accessibility report scaffolds** — extended the Button accessibility report pattern to every component detail page. Non-Button components now get structured reports with live active-theme contrast rows, known target-size token rows where available, keyboard expectations, screen reader expectations, WCAG mappings, test matrices, and manual checklists. Reports are classified as content, input, navigation, overlay, or interactive so the guidance matches the component type.

- **Accessibility page — component scan summary** — the `/accessibility` page now includes the grouped component accessibility scan alongside the global report. It shows component groups, stories scanned, affected components, total violations, and a searchable/filterable DataTable of per-component results generated from the latest component Storybook axe run.

- **Component detail pages — focused tabs and generated prop tables** — hid the Overview and Anatomy tabs across component detail pages so docs focus on Configure, Rules, Properties, and Accessibility. The Properties tab now prefers generated rows from `packages/react/src/components/**/*.d.ts`, including child prop interfaces such as tabs, table columns, menu items, and autocomplete options, with local rows kept as fallback for package surfaces that do not expose declarations yet. Specialized field pages (date, time, phone, ZIP, and credit card) now share the current Text Field prop surface. Fixed the Bleed preview contrast by applying the inverse text token to its paragraph sample.
...
- **CSS audit — replace custom styles with A1 components and utilities** (A1-295) — removed 107 lines from `styles.css` by replacing custom classes with system equivalents: `a1-web-alpha-badge` → `<MessageBadge status="info" size="sm">` in the settings menu; `a1-web-visually-hidden` → `a1-sr-only` utility (added `sr-only.css` to `main.jsx` imports); `a1-web-shortcuts-{table,row,label,keys}` → `<DefinitionList direction="row" size="sm">` + native `<kbd>` in the keyboard shortcuts dialog (added `inline.css` to `main.jsx` for system `kbd` styling); `a1-web-section-body` → `style={{ maxInlineSize: 'var(--base-content-width-xs)' }}` on the six `<Paragraph>` callout elements across Home, Features, Foundations, and ComponentsOverview; `a1-web-stat` → inline `paddingBlock` on the Home stats-strip Stack. Four additional classes (`a1-web-code-block`, `a1-web-inline-code`, `a1-web-category-card`, `a1-web-component-packages`) had no JSX consumers and were deleted outright.

- **Label translation hook — `useT()`** (A1-366) — added `apps/a1-web/src/labels/useT.js` exporting a `TContext` / `TProvider` / `useT()` pattern. `TProvider` wraps the app root in `main.jsx` and resolves strings through the existing label stack (system JSON → workspace Supabase/localStorage → project overrides). All pages now call `const t = useT()` and replace hardcoded strings with `t('key', 'fallback')` — giving the Label editor the ability to inspect and override any of these strings without a rebuild. Groundwork for broader app i18n; first namespace is `app.labelEditor.*`.

- **Label editor — full i18n** (A1-366) — all user-visible strings in LabelEditor and AddLabelDialog translated to `app.labelEditor.*` keys in `system/labels/app.json`. The new namespace adds 44 keys (page title, breadcrumb, toolbar actions, column headers, tab labels, empty states, dialog fields, error messages, and plural labels) each with translations for `es`, `fr`, `de`, `pt`, `ja`, `zh`, and `ar`. The workspace and system label grids, the Add label dialog, the Translate action, the JSON viewer, and the History tab are all i18n-ready.

- **Label editor — edit history** (A1-366) — workspace label changes are now recorded as history entries: a 4-second debounce fires after each edit and appends a snapshot (locales config + items) to `localStorage` under `a1-label-history-v1` (cap 50). When signed in, entries are also cloud-synced via the shared `appendHistory` / `fetchHistory` services (`entityType: 'labels'`, `entityId: 'workspace'`); cloud entries replace local ones when available. A History tab in the Label editor lists all entries via `EditorHistoryPanel` with restore and rename support. The empty state is owned by the tab rather than delegated to the panel so it shows immediately before the first edit, not after an entries-length threshold.

- **Editor labels — text and field label keys** — selected Heading, Paragraph, and Button text controls now show a label overflow menu beside their text/label field, and their canvas right-click menu now offers **Choose text label**. The field menu supports **Choose label** and **Add label**; Add label suggests a dot-notation key from the selected text, writes the new row to the shared workspace label dataset, and includes an Auto translate action for enabled locales. Choosing a label from the searchable single-select DataTable dialog stores `content.textKey` while the selected English value becomes the fallback; labeled text fields display as `<<label.key>>` so editors can tell the content is label-driven. TextField and TextareaField nodes also expose an optional Label key control with the same label menu for field labels.

- **Label editor** (A1-297) — new Editors page at `/label-editor` for managing workspace labels and translations. Features: a react-data-grid spreadsheet with editable key + per-locale columns; "Translate all missing" (and "Translate N selected") using the free MyMemory API to auto-fill empty cells from the English value; per-workspace locale configuration via a ChoiceGroup toggle panel (EN always on, AR flagged as right-to-left); Arabic cells render and edit with `dir="rtl"`; an "Add label" dialog with an Autocomplete that suggests existing system label keys (and auto-fills the English value when a match is chosen) plus an auto-translate-on-add toggle; "Export JSON" downloads the label set; the label store (`labels/labelStore.js`) uses the shared Supabase `workspace_labels` row when accounts are configured, with localStorage as the offline/local fallback. No system label files were modified — the editor manages workspace-level labels only.

- **TopHeader — action dividers** — `TopHeader` actions array now supports `{ divider: true, id }` items that render a 1px vertical separator (`a1-top-header__action-divider`) between action buttons. Used in `main.jsx` to separate the account action group from other header actions. No new prop on `TopHeader` itself — the divider is a first-class array item alongside normal action objects.

- **DataGrid — A1-styled checkboxes** — the react-data-grid selection checkbox column now uses A1 token-styled checkboxes instead of the native browser checkbox. Tokens: semantic border, action background on checked/indeterminate, focus ring, disabled surface. Appearance: `none` on the input; border, border-radius, background, and transition all from CSS custom properties. No prop change — applies to all DataGrid instances via `.a1-data-grid .rdg-checkbox-input` in `data-grid.css`.

## 0.15.0 — 2026-06-25

- **Virtual Icon Designer** (A1-348) — added two dev-only icon generation entry points that feed the custom icon font. **Dialog** (Custom icons page → "Design with AI" button): describe a concept; Claude Sonnet designs three 24×24 SVG variations to the Material Symbols filled-path spec, shows rendered previews with editable names, and lets you accept each directly into the font. Gated on `import.meta.env.DEV` and the user-configured Anthropic API key (`VITE_AI_ENABLED`). **Virtual team panel** (`backlog/VirtualIconPanel.jsx`): same workflow using a local Ollama model instead; geometric template fallback when no model is reachable. Both entry points validate SVG through `validateCustomIconSvg`, persist via `addCustomIcon`, and require no project selection. Old `VirtualIconDesigner.jsx` replaced by `VirtualIconDesignerDialog.jsx` + `VirtualIconPanel.jsx`; `designIcons()` restored in `lib/aiIcons.ts`; `services/virtualIcon/` added for the Ollama-backed service.
- **Virtual Information Architect — Ollama AI layer** — the Virtual Architect now runs a two-layer audit: (1) the existing 13 deterministic IA/UX heuristics (unchanged, always offline) and (2) an optional Ollama analysis that enriches findings with concrete label rename proposals and adds 1–3 contextual observations the rule engine cannot express (conceptual overlap, domain coherence, discoverability gaps). Ollama runs in the background when the dialog opens; heuristic findings appear immediately and AI enrichment appears when ready. Each AI observation can be filed as a backlog ticket or declined like heuristic findings, with the same persistent de-duplication. "AI enhanced" badge on the panel card when Ollama contributed. Falls back silently when no local model is available. New `services/architect/architectAi.ts`; `VirtualArchitectPanel.jsx` updated with `useEffect` AI trigger, `aiDecisions` hook, and two `useFindingDecisions` namespaces (`architect` + `architect-ai`).
- **Editors — project-scoped custom icon fonts** — added a Custom icons editor that accepts SVG uploads, enforces a safe 24×24 filled-path specification, stores icons locally in IndexedDB, and builds a TrueType icon font entirely in the browser. Custom names use the explicit `custom:<name>` namespace while unprefixed names continue to use Material Symbols. The active editor and standalone project preview register global icons plus icons scoped to the current project; icon configurators list only custom icons available to that project. The page supports project filtering, scope editing, and deletion, and seeds the supplied engine SVG as `custom:engine` for the Northstar Motors vehicle project.
- **Projects — Northstar Motors vehicle sample** — added a seeded two-page sample project for exercising the Vehicles dataset. Inventory repeats one navigation Card template over the first 24 vehicle rows, binding the image, powertrain, body style, availability, name, description, price, mileage, and item-specific details link. Vehicle details declares `detailDataset: "vehicles"` and presents the selected record through a hero and six-image gallery, at-a-glance metrics, and full specification groups for powertrain, battery/charging, dimensions and wheelbase, chassis/wheels/brakes, interior technology and speakers, airbags and driver assistance, warranties, and ownership estimates. Shared-layout composition now preserves the content page's identity and detail-dataset metadata, so standalone preview links resolve the selected `item` row instead of always showing the first record. Both pages explain that the Vehicles sample must be added from Data sources and that its generated values and illustrative stock photography are not manufacturer-certified.
- **Data sources — Vehicles sample dataset** — added a deterministic synthetic vehicle catalog with 120 records and a wide specification schema spanning identity, pricing, ownership estimates, gasoline/diesel/hybrid/PHEV/EV/hydrogen powertrains, engine bore and stroke, motor/battery/charging data, performance, efficiency, dimensions, wheelbase, payload/towing, chassis, wheels, tires, brakes, exterior equipment, interior comfort, infotainment, speaker counts, airbags, driver assistance, warranty, and eight illustrative image fields per vehicle. The catalog covers passenger cars, sports cars, crossovers, SUVs, pickups, a minivan, and a commercial van across model years 1998–2026. It is offered as the new Vehicles built-in sample and explicitly identifies its values and stock photography as realistic demo data rather than manufacturer-certified specifications.
- **Virtual data teammate** (A1-344) — added a fourth dev-only Virtual team member that connects existing project pages to available data sources. It optionally asks the configured local Ollama model to choose the most relevant dataset for each page, then uses a deterministic validator to propose only supported page-definition connections: repeating a single Card template in a Grid or ListItem template in a List, binding matching text and props with existing `{{ dataset.column }}` tokens, and filling DefinitionList items from the active row. Every run opens a preview listing the exact page-level changes; nothing writes until Apply. Applied changes are stored as named editor-history entries and sync through the existing project storage path. When no local model is reachable, explainable local matching provides the fallback. Built from existing A1 components; no design-system API changed.
...
- **Main menu — top-level groups now have icons** (A1-262) — the top-level groups in the main navigation (Explore, Foundations, Components, Editor) were the only nav entries without an icon, while their child items and the action buttons all had one. Each group now carries a Material Symbols icon (`menu_book` / `foundation` / `widgets` / `design_services`), so the menu IA is consistent in both the desktop TopHeader (icon before the label, the default `navIconPosition="start"`) and the mobile SideNav drawer. Data-only change in `main.jsx` `navItems` — the icons are decorative (`aria-hidden`; the visible label remains the accessible name). No design-system component changed.
...
- **Build with AI now includes linked tickets** (A1-283) — the ticket dialog's **Build with AI** tab feeds the ticket's **linked tickets** (A1-218) into the plan, so related work that should ship together isn't planned in isolation. The plan context (and the deterministic fallback) gain a **Linked tickets** section listing each linked ticket's ref, type/status, title, and a short description, with a nudge to check whether they share scope or should ship together. Resolved in `TicketAiPrompt.jsx` from `item.links` against the loaded backlog, threaded through `buildTicketContext` / `buildPlanRequest` / `developPlanLocally` (`services/backlog/devPlan.ts`). Local + deterministic — no API/credits; no design-system component changed.
...
- **Virtual PO — smarter, non-repeating questions + moves shipped tickets** (A1-209) — fixes the core complaint that the PO "asks a question, I answer, and then it just repeats the question," and makes its questions context-aware.
  - **No more repeats.** The runner (`src/services/backlog/personas/runPersona.ts`) now reads a ticket's thread before asking and **never re-asks a question it has already asked** — matched by a stable `key` on each question _or_ by exact text (so legacy questions without keys are still de-duplicated). It asks **at most two** questions at a time, and **none** while a question is still awaiting your answer. Once you answer and the ticket changes, it asks the _next_ unasked questions instead of looping.
...
- **Editor — 7 more components addable** — the editor Add panel and renderer now support **Slider, Tabs, Toolbar, Field Row, Page Nav, Tree Menu, and Data Table**. Registered in `editor/pageTypes.ts` + `editor/componentRegistry.ts`, with catalog entries (`editor/componentCatalog.ts`) and config bridges (`editor/EditorPropsPanel.jsx`) that reuse each component's existing detail-page `Controls`. **Slider** and **Page Nav** render the real components with real props; **Field Row** is a real container holding field children (added to `CONTAINER_TYPES`); **Tabs, Toolbar, Tree Menu, and Data Table** render via thin **editor adapters** (`editor/editorComponents.jsx`) that reuse the detail-page `Preview` (their node `props` carry the configurator config, since these are compositional or render fixed sample data). _Known limitation:_ the editor's **React-code** export serializes the adapter config rather than the literal `<Tabs>`/`<Toolbar>`/`<DataTable>` composition — the component detail page remains the canonical code source for those. Still deferred per request: Inline, Inline Editable, Side Nav, Page Layout, Dialog, Menu, Context Menu, Snackbar, Notification.

- **Canonical component icons (source of truth)** — every A1 component now has one canonical Material Symbols icon, defined on `componentCategories[].components[].icon` in `src/pages/components/data.js` (also exported as `COMPONENT_ICONS`). The editor **Add** panel catalog icons were aligned to it, and `system/icons/icon-usage.md` gained a full **Component Icons** table, so the same icon represents a component everywhere (component browser, Add panel, layers tree). _(Foundation for the "all components addable in the editor" work; the catalog category reorg to the new taxonomy and editor render support for the components not yet renderable there — Inline, Slider, Toolbar, Sticky Actions, Accordion, Tabs, Side Nav, Page Nav, Tree Menu, Field Row, Inline Editable, Notification, Snackbar, Dialog, Menu, Context Menu, Data Table, Page Layout — remain as follow-up.)_
