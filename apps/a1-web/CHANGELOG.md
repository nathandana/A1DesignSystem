...
- **Configuration panel as right sidebar** — replaced Grid layout with flex layout; configuration controls render in a fixed-width (280px) right sidebar. On the Configure tab the docs shell enters a "fill" mode (`a1-web-components-shell--fill`) that makes the page fill the scroll viewport via a flex chain — the breadcrumb/title/tabs stay pinned and the preview column and config sidebar each scroll internally to the bottom of the viewport, matching the left sidebar's behavior. No fixed-height magic numbers.
