...
- **Backlog board horizontal scroll strips** (A1-194) — the desktop backlog board now wraps its swimlane grid in a keyboard-focusable horizontal scroll strip. Swimlane `Section`s use token-backed `xs` widths and the grid tracks use the same content-width token, so cards stay inside their lane boundaries while extra lanes scroll inside the board instead of causing page-level horizontal overflow.
