...
- **Foundations — System map color architecture** — expanded `/foundations/system-map` to show the actual token and theming pipeline uncovered by the color-system audit. The graph now separates base/semantic/component/brand aliases, namespaced DTCG theme tokens, raw `theme.json` selector overrides, Style Dictionary, the custom theme and HTML/CSS builders, generated `tokens.css`/`tokens.json`/`themes.css`, hand-authored `color-scheme.css`, runtime theme/mode classes, and inherited inverse islands before they reach packages and components.
