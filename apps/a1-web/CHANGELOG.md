...
- **Backlog views — a segmented control in the panel, not top tabs** — the top-level **Board / All tickets / My queue / Virtual team** tabs were replaced by a **`SegmentedControl` at the top of the right-hand panel** (icon segments, full-width, labelling only the selected view). The main area renders the active view directly. The panel's **Sort + filters now show only on the Board view** (they're board-specific); the search and the New ticket footer stay across all views. `src/pages/Backlog.jsx` — built from existing A1 components, no design-system component changed.
