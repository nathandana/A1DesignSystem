...
- **Main menu — top-level groups now have icons** (A1-262) — the four top-level groups in the main navigation (Resources, Foundations, Components, Editor) were the only nav entries without an icon, while their child items and the action buttons all had one. Each group now carries a Material Symbols icon (`menu_book` / `foundation` / `widgets` / `design_services`), so the menu IA is consistent in both the desktop TopHeader (icon before the label, the default `navIconPosition="start"`) and the mobile SideNav drawer. Data-only change in `main.jsx` `navItems` — the icons are decorative (`aria-hidden`; the visible label remains the accessible name). No design-system component changed.
