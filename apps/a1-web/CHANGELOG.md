...
- **Backlog filters — moved into a panel, persist + clear all** (A1-154) — the board's Type / Priority / Size / Scope filters **moved off the toolbar into a “Filters” panel** (a `Dialog`) opened by a **“Filters (n)”** button that shows how many are active; the board toolbar is now just that button plus the view controls (Sort, Swimlanes). The filters also **persist across reloads** (saved to `localStorage` as `a1-backlog-filters`, with the sort and search), so a working view sticks, and a **“Clear filters (n)”** action (on the board when active, and in the panel footer) resets every filter + the search in one click. No new filter dimensions and no overlap with the My-queue tab. `src/pages/Backlog.jsx` only (`DEFAULT_FILTERS` / `loadFilterState` / `countActiveFilters` + a persist effect + a `Dialog` panel); built from existing A1 components — no design-system component changed.
