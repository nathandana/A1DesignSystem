...
- **Virtual Icon Designer** (A1-348) — added an AI-powered icon generator to the Custom icons page. Click "Design with AI" to describe a concept; Claude Sonnet designs three SVG icon variations to the Material Symbols 24×24 filled-path spec, shows rendered previews with suggested names, and lets you accept each design directly into the custom icon font with a name edit and optional project scope. Re-prompt or regenerate to iterate. The feature is gated on `VITE_AI_ENABLED` and the user-configured Anthropic API key; the button is hidden when AI is off.
