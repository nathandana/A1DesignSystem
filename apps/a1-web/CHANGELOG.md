...
- **Figma Radio Group component** (A1-1403) — Radio Group is now marked available in the Components inventory's Figma coverage. The A1 Figma parent set (`node 283:1121`) covers default, comfortable, and compact densities, with a matching composable Radio Option set for option-level size, selected, and hover states. Both use shared light/dark Color-mode bindings, examples, and a checked-in Code Connect template.
