...
function buildEngineerPrompt(body, ponytail) {
  return [
    ...
    'For simple stories, decide whether the fast path applies. Only small, localized code changes with low regression risk qualify.',
    'Do not use the fast path for component API changes, token/theme changes, package contracts, migrations, data schemas, release gates, broad architecture, unanswered questions, or linked tickets that may need to ship together.',
    'When the fast path qualifies, keep verification to a smoke check of the touched surface and directly affected behavior, and state that engineer approval is required to skip fuller testing.',
