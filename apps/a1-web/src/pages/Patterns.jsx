...
<Breadcrumb
        ...
        />
            ...
            <Paragraph size="sm" color="muted">
              Reusable, governed compositions — a page header, a footer, or a standardized
              card. Open one to edit it and lock the parts that should stay consistent.
            </Paragraph>
    ...
    <Section padding="sm" aria-labelledby="patterns-heading" contentWidth="xl">
      <Stack direction="column" gap="sm">
        ...
        {patterns.length === 0 ? (
          <MessageEmptyState
            ...
            description="Patterns are reusable compositions you can insert into pages and govern with locks."
            action={<Button icon="add" onClick={handleCreate}>New pattern</Button>}
          />
        ) : (
                    ...
                    <Stack direction="column" gap="sm">
                      ...
                      <DefinitionList
                        ...
                        items={[
                          ...
                          { label: 'Locked', value: `${lockedComponents} components · ${lockedProps} props` },
                          { label: 'Editable', value: `${editableFields} fields` },
                          { label: 'Scope', value: meta.projectIds?.length ? `${meta.projectIds.length} project${meta.projectIds.length > 1 ? 's' : ''}` : 'All projects' },
                        ]}
                      />
                    </Stack>
                  </Card>
      ...
      <ContextMenu
        ...
        items={menu ? [
          { id: 'open', label: 'Open', icon: 'edit', onClick: () => { openPattern(menu.pattern.id); setMenu(null) } },
          { id: 'dup', label: 'Duplicate', icon: 'content_copy', onClick: () => { duplicatePattern(menu.pattern.id); setMenu(null); refresh() } },
          ...(isUserPattern(menu.pattern.id)
            ? [{ type: 'divider', id: 'd1' }, { id: 'del', label: 'Delete', icon: 'delete', variant: 'destructive', onClick: () => { setConfirmDelete(menu.pattern); setMenu(null) } }]
            : []),
        ] : []}
      />
      ...
      <Dialog
        ...
        footer={
          <>
            ...
            <Button variant="destructive" icon="delete" onClick={() => { deletePattern(confirmDelete.id); setConfirmDelete(null); refresh() }}>Delete</Button>
          </>
        }
      >
